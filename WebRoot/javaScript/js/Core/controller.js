/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
 1. 项目名称：www.qiekanginc.com
 2. 页面名称：Controller (每个页面的类都继承于这个控制器基类)
 3. 作者：(leilangfeng@guanaihui.com)
 4. 备注：对api的依赖：jQuery
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
function Controller() {
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    对环境的定义：
    @development : 开发环境，对应静态资源域名为：source01.qiekanginc.com - source20.qiekanginc.com
    @test：测试环境，对应静态资源域名为：stage01.qiekanginc.com - stage20.qiekanginc.com
    @preview：预发布环境，对应静态资源域名为：preview01.qiekanginc.com - preview10.qiekanginc.com
    @production ：生产环境，对应静态资源域名为：cdn01.qiekanginc.com - cdn20.qiekanginc.com
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.environment = "production"; //环境定义  
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    一些关于cookie参数的配置
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.cookieDomain = this.environment === "production" ? ".qiekanginc.cn" : ".qiekanginc.com"; //整个app用来设置和取得cookie的域名  
    ...
    this.staticDomain = "//dev" + randomDomainSn() + ".qiekanginc.com";
    if (this.environment === "test") {
        this.staticDomain = "//stage" + randomDomainSn() + ".qiekanginc.com";
    } else if (this.environment === "preview") {
        this.staticDomain = "//preview" + randomDomainSn() + ".qiekanginc.com";
        this.cookieDomain = ".qiekanginc.cn";
    } else if (this.environment === "production") {
        this.staticDomain = "//cdn" + randomDomainSn() + ".qiekanginc.com";
        this.cookieDomain = ".qiekanginc.com";
    }
    ...
    this.appStaticPrefix = this.staticDomain + "/www.qiekanginc.com/js";
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    根据环境决定接口地址的前缀
    -------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.apiPrefix = "//devweb.guanaihui.cn";
    if (this.environment === "test") {
        this.apiPrefix = "//testweb.guanaihui.cn";
    } else if (this.environment === "preview") {
        this.apiPrefix = "//previewweb.guanaihui.cn";
    } else if (this.environment === "production") {
        this.apiPrefix = "//mobile.guanaihui.com";
    }
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    /*且康官网接口*/
    this.kindListUrl=this.apiPrefix+"/information/in/kindList";  //获取类型列表
    this.kindConListUrl=this.apiPrefix+"/information/in/list";  //获取类型对应的列表/
    this.articleDetailUrl=this.apiPrefix+"/information/in/detail";  //获取文章详情
