		...
		(function(isWindow, global) {
			...
			_IDValidator.prototype = {
				...
				isValid : function(id, forceType) {
					...
					var birth = (code.type === 18 ? code.body.slice(6, 14)
							: code.body.slice(6, 12));
					var order = code.body.slice(-3);

					if (!(util.checkAddr(addr, GB2260)
							&& util.checkBirth(birth) && util.checkOrder(order))) {
					...
					checkBit = (typeof checkBit === 'number' ? checkBit
							.toString() : checkBit);
